...
call(["rm", "results_10x_epochs/test_ll.txt"])
call(["rm", "results_10x_epochs/test_rmse.txt"])
call(["rm", "results_10x_epochs/test_MC_rmse.txt"])
call(["rm", "results_10x_epochs/time.txt"])
call(["rm", "results_10x_epochs/log.txt"])
...
for i in range(n_splits):
    ...
    network = net.net(X_train, y_train,
        [ n_hidden ], normalize = True, n_epochs = int(n_epochs*100), X_test=X_test, y_test=y_test)
    ...
    with open("results_10x_epochs/test_rmse.txt", "a") as myfile:
        myfile.write(repr(error) + '\n')

    with open("results_10x_epochs/test_MC_rmse.txt", "a") as myfile:
        myfile.write(repr(MC_error) + '\n')

    with open("results_10x_epochs/test_ll.txt", "a") as myfile:
        myfile.write(repr(ll) + '\n')

    with open("results_10x_epochs/time.txt", "a") as myfile:
...
with open("results_10x_epochs/log.txt", "a") as myfile:
